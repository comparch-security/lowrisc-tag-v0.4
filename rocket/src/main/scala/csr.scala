...
class CSRFile(id:Int)(implicit p: Parameters) extends CoreModule()(p)
{
  ...
  val reg_L2D_write_miss = if(usingPFC) RegNext(next = WideCounter(64,PopCount((Range(0,p(PFCL2N))).map{io.pf.Cache.L2D(_).write_miss})).value) else UInt(0)
  ...
  if(usingPFC) {
    ...
    read_mapping += CSRs.L2missw -> (reg_L2D_write_miss,           UInt(0))
