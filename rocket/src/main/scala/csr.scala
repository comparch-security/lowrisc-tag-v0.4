...
class CSRFile(id:Int)(implicit p: Parameters) extends CoreModule()(p)
{
  ...
  when (wen) {
    ...
    if (usingPFC) {
      ...
      when (decoded_addr(CSRs.pfcc)) {
        reg_pfcc     := Cat(wdata(63,60), wdata(59,52), wdata(51,10), wdata(9,4), Bool(false), Bool(true), Bool(false), wdata(0))
        //reg_pfcc   := Cat(mode,         pfcMtype,       pfcMID,     reserve,    read_error,  empty,      interrupted,  trigger)
  ...
  if (useTagMem) {
    ...
    when(reg_mstatus.prv =/= PRV.U) {
      ...
      io.tag_ctrl.maskFetchChck := UInt(0)
      io.tag_ctrl.maskCFlowDirBranchTgt   := UInt(0)
      io.tag_ctrl.maskCFlowIndirBranchTgt := UInt(0)
    }  //only check tag in U mode
  } else {
