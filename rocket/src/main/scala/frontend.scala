...
class Frontend(implicit p: Parameters) extends CoreModule()(p) with HasL1CacheParameters {
  ...
  io.cpu.pfc.read := Reg(next = io.cpu.resp.fire() && addrMap.isCacheable(icache_pa))
  io.cpu.pfc.read_miss := Reg(next = io.mem.acquire.fire() && addrMap.isCacheable(icache_pa))  //exclude io
}
