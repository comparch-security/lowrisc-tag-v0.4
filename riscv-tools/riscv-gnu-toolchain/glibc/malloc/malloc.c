...
#define get_tag(dst) ({\
  ...
  __tmp2;})
#define RV64_MALLOC_TAG 0x1
#define RV64_TAG_ALIGN_DOWN(base,size) ((void *)(((unsigned long)(base)) & ~((size) - 1)))
#define RV64_TAG_ALIGN_UP(base,size) ((void *)((((unsigned long)(base)) + ((size) - 1)) & ~((size) - 1)))

static inline void addtag(void * start, size_t bytes){
  void * start_aligned = RV64_TAG_ALIGN_DOWN((unsigned long)start,(8lu));
  void * end_aligned = RV64_TAG_ALIGN_UP((unsigned long)(start+bytes),(8lu));
  // fprintf(stderr,"addtag start from %p, end at %p :\n",start_aligned, end_aligned);
  for (uint64_t * p = start_aligned ; p < (uint64_t*)end_aligned; p += 1){
    set_tagged_val(p,*p,RV64_MALLOC_TAG);
    // fprintf(stderr,"tag @%p is %ld\n",p,get_tag((uint64_t*)p));
  }
}

static inline void rmtag(void * start) {
  size_t bytes = malloc_usable_size(start);
  void * start_aligned = RV64_TAG_ALIGN_DOWN((unsigned long)start,(8lu));
  void * end_aligned = RV64_TAG_ALIGN_UP((unsigned long)(start+bytes),(8lu));
  // fprintf(stderr,"rmtag start from %p, end at %p :\n",start_aligned, end_aligned);
  for (uint64_t * p = start_aligned ; p < (uint64_t*)end_aligned; p += 1){
    set_tagged_val(p,*p,0);
    // fprintf(stderr,"tag @%p is %ld\n",p,get_tag((uint64_t*)p));
  }
}

static int _malloc_tag_hook_inited = 0;
static int _free_tag_hook_inited = 0;
static void* (*_malloc_tag_hook_saved) (size_t, const void *);
static void (*_free_tag_hook_saved) (void* , const void *);
void* _malloc_tag_hook (size_t, const void *);
void _free_tag_hook (void* , const void *);

void _malloc_tag_initialize_hook (void) 
{
  if ((!_malloc_tag_hook_inited) && (!_free_tag_hook_inited)){
    _malloc_tag_hook_saved = __malloc_hook;
    _free_tag_hook_saved = __free_hook;
    __malloc_hook = _malloc_tag_hook;
    __free_hook = _free_tag_hook;

    _malloc_tag_hook_inited = 1;
    _free_tag_hook_inited = 1;
  }
}

void * _malloc_tag_hook (size_t size, const void * caller)
{
  // restore all old hooks
  __malloc_hook = _malloc_tag_hook_saved;
  __free_hook = _free_tag_hook_saved;
  // recursive call
  void* retval = __libc_malloc (size);
  // save underlying hooks
  _malloc_tag_hook_saved = __malloc_hook;
  _free_tag_hook_saved = __free_hook;
  // do what you want.
  // add tag RV64_MALLOC_DEFAULT to all memory units allocated.
  addtag(retval,size);
  // fprintf(stderr,"mem alloca@%p, size %#lx, tagged with tag %ld.\n",retval,bytes,get_tag((uint64_t*)retval));
...
void  _free_tag_hook (void* ptr, const void * caller)
{
  // do what you want.
  rmtag(ptr);
  // restore all old hooks
...
__libc_malloc (size_t bytes)
{
  ...
  if(!_malloc_tag_hook_inited)
    _malloc_tag_initialize_hook();
...
# endif
  ...
  //add tag for newly allocated memory
  addtag(mem,sz);
  return mem;
}
